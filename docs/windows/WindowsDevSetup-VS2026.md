...
Visual Studio 2026으로 AluxLabs Link를 빌드/디버깅하기 위한 환경 세팅 절차.

## 0. 사전 정보

- 솔루션 파일 `aluxlabs-link.sln`은 VS 2026에서 그대로 열 수 있다. **버전 변환 프롬프트가 떠도 변환하지 말 것** (sln 포맷이 바뀌어 PR이 지저분해진다).
- 윈도우 관련 프로젝트:
...
- ☑ **Windows 11 SDK (10.0.22621.0)** — `aluxlabs-link-win.csproj`의 `TargetFramework=net8.0-windows10.0.22621.0`이 요구하는 SDK.
...
| Startup Project | **`aluxlabs-link-win`** |

> **시작 프로젝트는 반드시 `aluxlabs-link-win`이어야 한다.** `aluxlabs-link-win-msix`로 F5를 누르면 패키지 ID 충돌로 `MddBootstrapInitialize 0x80070032` 에러가 난다.
...
| `aluxlabs-link-win-msix`가 회색으로 비활성화 | Configuration이 `*_Win`이 아닌 다른 것으로 되어 있음. `Debug_Win` 또는 `Release_Win`으로 전환. |
| CS 빌드 에러 (System.Management, Fleck 등 누락) | NuGet 캐시 불일치. CLI에서 `dotnet restore --force aluxlabs-link-win/aluxlabs-link-win.csproj` 실행 후 VS 재시작. |
